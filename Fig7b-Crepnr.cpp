// Fig7b-Crepnr.cpp: 主项目文件。
//"Circular at the very beginning: On the initial genome in the RNA world"
//by Yufan Luo, Minglun Ling, Chunwu Yu, Wentao Ma*
//C source codes for the simulation program -- the version corresponds to the case shown in Fig. 7b and Fig. 8 of the paper
...
#define STEPNUM 5000000     // Total time steps of Monte Carlo simulation
...
#define CTSEQ     A,C,U,G,A,C,G,U    // The characteristic sequence of the control species 
#define CTCOMSEQ  A,C,G,U,C,A,G,U   // The complement of the characteristic sequence of the control species

#define REPNRSEQ REPSEQ,NRSEQ  
#define REPNRCOMSEQ NRCOMSEQ,REPCOMSEQ

#define BCLENGTH (replength+nrlength)

#define BCCTSEQ CTSEQ,U,G,A,C,G,C,A,G
#define BCCTCOMSEQ C,U,G,C,G,U,C,A,CTCOMSEQ

#define INOCUSEQ REPSEQ,U,G,A,C,G,C,A,G    // The inoculated species: REP + noncoding sequence (the noncoding sequence differs from NR sequence by only 1 nt)
#define INOCUSEQ1 CTSEQ,U,G,A,C,G,C,A,G    // Another inoculated species: CT + noncoding sequence

#define INOCUNUM 100           // The molecular number of the inoculated species
#define INOCUSTEP 10000       // The step for the inoculation

#define SD 31				  // Random seed
#define N 30				  // The side length of the two-dimensional grid
#define TOTAL_MATERIAL 200000 // Total materials in the system

#define PSP 0.5            // Probability of separation of a base-pair
#define PBB  0.00002      // Probability of breaking of a phosphodiester bond
...
#define PNF 0.001          // Probability of a nucleotide forming from a nucleotide precursor
...
#define PNDE 0.002         // Probability of a nucleotide residue decaying at RNA's chain end
...
static char repnrseq[50] = { REPNRSEQ };        // Presumed nt_synthetase sequence
static char repnrcomseq[50] = { REPNRCOMSEQ };
static char bcctseq[50] = { BCCTSEQ };
static char bcctcomseq[50] = { BCCTCOMSEQ };
...
float repnr[(STEPNUM - STAREC) / RECINT + 1];  // Record number of nt-synthetases in steps
float cir_repnr[(STEPNUM - STAREC) / RECINT + 1];
float repnrcom[(STEPNUM - STAREC) / RECINT + 1];  // Record number of nt-synthetases in steps
float cir_repnrcom[(STEPNUM - STAREC) / RECINT + 1];

float bcct[(STEPNUM - STAREC) / RECINT + 1];  // 
float cir_bcct[(STEPNUM - STAREC) / RECINT + 1];  //
float bcctcom[(STEPNUM - STAREC) / RECINT + 1];  // 
float cir_bcctcom[(STEPNUM - STAREC) / RECINT + 1];  //
...
void record(void)               // Data recording
{
	...
	for (y = 0; y < N; y++)
	{
		for (x = 0; x < N; x++)
		{
			...
			for (p = room_head[h][y][x]->next; p != room_head[h][y][x]; p = p->next)
			{
				...
				flag = findseq(repnrseq, BCLENGTH, p);
				if (flag == 0) 
				{
					if(p->type1==0)	repnr[g]++;
					else cir_repnr[g]++;
				}
				flag = findseq(repnrcomseq, BCLENGTH, p);
				if (flag == 0) 
				{
					if(p->type1==0) repnrcom[g]++;
					else cir_repnrcom[g]++;
				}

				flag = findseq(bcctseq, BCLENGTH, p);
				if (flag == 0) 
				{
					if(p->type1==0)	bcct[g]++;
					else cir_bcct[g]++;
				}
				flag = findseq(bcctcomseq, BCLENGTH, p);
				if (flag == 0) 
				{
					if(p->type1==0) bcctcom[g]++;
					else cir_bcctcom[g]++;
				}
	...
	printf("----- step=%d: nr=%d-%d|%d (%d|%d), rep=%d-%d|%d (%d|%d), repnr=%d|%d (%d|%d), bcct=%d|%d (%d|%d),ct=%d|%d (%d|%d), unit=%d (%d)  (tn=%d, r=%d)\n", i,        //Ma-new5
		(int)nr[g],(int)nr_rib[g],(int)nrcom[g], (int)cir_nr[g],(int)cir_nrcom[g], (int)rep[g],(int)rep_rib[g],(int)repcom[g], (int)cir_rep[g],(int)cir_repcom[g], 
		(int)repnr[g],(int)repnrcom[g],(int)cir_repnr[g], (int)cir_repnrcom[g],     (int)bcct[g],(int)bcctcom[g], (int)cir_bcct[g],(int)cir_bcctcom[g],
		 (int)ct[g],(int)ctcom[g], (int)cir_ct[g],(int)cir_ctcom[g],
		(int)unit[g], (int)cir_unit[g], (int)total_mat_num[g], (int)np_num[g]);
	fprintf(fptxt, "----- step=%d: nr=%d-%d|%d (%d|%d), rep=%d-%d|%d (%d|%d), repnr=%d|%d (%d|%d), bcct=%d|%d (%d|%d),ct=%d|%d (%d|%d), unit=%d (%d)  (tn=%d, r=%d)\n", i,        //Ma-new5
		(int)nr[g],(int)nr_rib[g],(int)nrcom[g], (int)cir_nr[g],(int)cir_nrcom[g], (int)rep[g],(int)rep_rib[g],(int)repcom[g], (int)cir_rep[g],(int)cir_repcom[g], 
		(int)repnr[g],(int)repnrcom[g],(int)cir_repnr[g], (int)cir_repnrcom[g],     (int)bcct[g],(int)bcctcom[g], (int)cir_bcct[g],(int)cir_bcctcom[g],
		 (int)ct[g],(int)ctcom[g], (int)cir_ct[g],(int)cir_ctcom[g],
		(int)unit[g], (int)cir_unit[g], (int)total_mat_num[g], (int)np_num[g]);
	fprintf(fptxt1,"----- step=%d: nr=%d-%d|%d (%d|%d), rep=%d-%d|%d (%d|%d), repnr=%d|%d (%d|%d), bcct=%d|%d (%d|%d),ct=%d|%d (%d|%d), unit=%d (%d)  (tn=%d, r=%d)\n", i,        //Ma-new5
		(int)nr[g],(int)nr_rib[g],(int)nrcom[g], (int)cir_nr[g],(int)cir_nrcom[g], (int)rep[g],(int)rep_rib[g],(int)repcom[g], (int)cir_rep[g],(int)cir_repcom[g], 
		(int)repnr[g],(int)repnrcom[g],(int)cir_repnr[g], (int)cir_repnrcom[g],     (int)bcct[g],(int)bcctcom[g], (int)cir_bcct[g],(int)cir_bcctcom[g],
		 (int)ct[g],(int)ctcom[g], (int)cir_ct[g],(int)cir_ctcom[g],
		(int)unit[g], (int)cir_unit[g], (int)total_mat_num[g], (int)np_num[g]);
	...
	for (y = 0; y < N; y++)
	{
		for (x = 0; x < N; x++)
		{
			for (p = room_head[0][y][x]->next; p != room_head[0][y][x]; p = p->next)
			{
				...
				//flag3 = findseq(contrseq1, contrlength1, p);  debug-2-于佳凝
...
int main()
{
	...
	{
		if (i == INOCUSTEP)inoculate(1);  // Inoculating RNA species
